...
const pageScopeLoaders = {
  'banmanager/api': async () => {
    ...
    // v8+ ships a single cross-platform `BanManagerAPI` artifact. The
    // per-platform `versions` keys are kept for backwards-compatible
    // template references.
    const versions = {
      api: version,
      bukkit: version,
    ...
    // Fetch events list from the new public API module.
    const playerEventsRes = await fetch('https://api.github.com/repos/BanManagement/BanManager/contents/api/src/main/java/me/confuser/banmanager/api/event/player')
    const ipEventsRes = await fetch('https://api.github.com/repos/BanManagement/BanManager/contents/api/src/main/java/me/confuser/banmanager/api/event/ip')
    const nameEventsRes = await fetch('https://api.github.com/repos/BanManagement/BanManager/contents/api/src/main/java/me/confuser/banmanager/api/event/name')
    const [playerJson, ipJson, nameJson] = await Promise.all([playerEventsRes.json(), ipEventsRes.json(), nameEventsRes.json()])
    const toName = (a) => a.name.replace('.java', '')
    const events = []
      .concat(Array.isArray(playerJson) ? playerJson.map(toName) : [])
      .concat(Array.isArray(ipJson) ? ipJson.map(toName) : [])
      .concat(Array.isArray(nameJson) ? nameJson.map(toName) : [])
